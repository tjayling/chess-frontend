...
const ChessBoard: React.FC<Props> = ({ boardSize, playingWhite, encodedBoard, possibleMoves }: Props) => {
  ...
  useEffect(() => {
    ...
    if (finalMove?.getTargetSquare() === 24) {
      encodedBoard = 'rnbqkbnr/1pppppPp/8/p7/8/8/PPPPPPpP/RNBQKBNR';
    }
    return () => {
  ...
  const drawPieces = (p5: p5Types) => {
    ...
    for (let char of encodedBoard) {
      ...
      let xPos = playingWhite ? xIndex * squareSize + imageOffset : boardSize - ((xIndex + 1) * squareSize - imageOffset);
      let yPos = playingWhite ? yIndex * squareSize + imageOffset : boardSize - ((yIndex + 1) * squareSize - imageOffset);
